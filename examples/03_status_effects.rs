...
fn main() {
    ...
    let initial_def = status_manager.get_active_resolver(&base_resolver)
        .resolve(&def_id, &ctx).unwrap();
    ...
    let sunder_armor = StatusEffect {
        ...
        bonuses: vec![Bonus::add(def_id.clone()).flat(-15.0).in_phase(zzstat::transform::TransformPhase::Additive)],
        max_stacks: 3,
    ...
    let def_1 = status_manager.get_active_resolver(&base_resolver).resolve(&def_id, &ctx).unwrap();
    ...
    let def_2 = status_manager.get_active_resolver(&base_resolver).resolve(&def_id, &ctx).unwrap();
    ...
    let def_final = status_manager.get_active_resolver(&base_resolver).resolve(&def_id, &ctx).unwrap();
