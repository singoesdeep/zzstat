...
fn main() {
    ...
    let weapon = game_data
        .weapons
        .get(&weapon_vnum)
        .unwrap_or(game_data.weapons.values().next().unwrap());
    ...
    for m_id in monster_ids {
        ...
        attacker.register_source(
            ...
            Box::new(ConstantSource(crit_chance)),
        );
        attacker.register_source(
            StatId::from("PIERCING_CHANCE"),
            Box::new(ConstantSource(piercing_chance)),
        );

        defender.register_source(
            StatId::from("DEFENSE"),
        ...
        let dmg_pierce = CombatEngine::evaluate(
            &formula,
            &mut attacker,
            &ctx,
            &mut defender,
        ...
        let dmg_crit_pierce = CombatEngine::evaluate(
            &formula,
            &mut attacker,
            &ctx,
            &mut defender,
